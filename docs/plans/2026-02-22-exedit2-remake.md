...
- 進行中:
  - Task 4 GPU バックエンド再設計（DX11 実装 + DX12 PoC 比較/ベンチ拡張 + 履歴CSV/回帰チェック自動化 + compute経路GPU優先化 + fullframe 3x3 compute 実装 + DX11/DX12ベンチ履歴収集 + DX12内訳計測 + HRESULT診断で無効HLSL修正 + preflight一回化）
