...
5. GPU 利用不可環境で CPU フォールバックが働く（環境を用意できない場合は `SKIP` 記録を許容）。
...
4. 一時的に GPU モードを無効化できる環境でフォールバック動作を確認する。環境がない場合は `SKIP` を記録する。
...
4. 本プロジェクトのリリース仕様では、`fallback=SKIP` は許容値として扱う。
