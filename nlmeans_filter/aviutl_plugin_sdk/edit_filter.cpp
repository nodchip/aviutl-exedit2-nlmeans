//----------------------------------------------------------------------------------
//		サンプル編集プラグイン(フィルタプラグイン)  for AviUtl ver0.96c以降
//----------------------------------------------------------------------------------
#include <windows.h>
...
void disp( HWND hwnd,FILTER *fp,void *editp,int n,int l,int w,int h )
{
	...
	if( fp->exfunc->is_editing(editp) && l ) {
		...
		wsprintf(b,"%s  [%d/%d]  %s %s 周期:%d  ",filter.name,n+1,l,fp->exfunc->get_config_name(editp,fs.config),inter_txt[fs.inter],fs.index24fps);
