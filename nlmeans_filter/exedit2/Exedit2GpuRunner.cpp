...
bool Exedit2GpuRunner::ensurePipeline()
{
	...
	static const char* shaderSource =
		...
		"	[unroll]\n"
		"	for (int i = 0; i < 9; ++i){\n"
		"		const int cx = clampi(x + PatchOffsets[i].x, 0, widthMax);\n"
		"		const int cy = clampi(y + PatchOffsets[i].y, 0, heightMax);\n"
		...
		"				[unroll]\n"
		"				for (int i = 0; i < 9; ++i){\n"
		"					const int tx = clampi(sx + PatchOffsets[i].x, 0, widthMax);\n"
		"					const int ty = clampi(sy + PatchOffsets[i].y, 0, heightMax);\n"
